...
class ProblemManager:
    ...
    def create(self, slug: str, title: str, owner_id: int,
               time_limit_ms: int = 1000, memory_limit_mb: int = 256,
               input_file: str = "stdin", output_file: str = "stdout",
               is_private: bool = True) -> Problem:
        existing = Problem.find_by_slug(self.conn, slug)
        if existing:
            raise ProblemError(f"Problem with slug '{slug}' already exists")

        problem = Problem.create(
            self.conn, slug=slug, title=title, owner_id=owner_id,
        ...
        layout.init_problem_dir(
            slug, title, time_limit_ms, memory_limit_mb,
            input_file, output_file, is_private,
        )
        return problem
